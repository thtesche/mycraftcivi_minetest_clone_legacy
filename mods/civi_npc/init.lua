...
mobs:register_mob("civi_npc:lumberjack", {
    ...
    mesh = "character.b3d",
    textures = {
        { "blank.png", "character.farmer_male.png", "blank.png", "blank.png" }
    },
    ...
    do_custom = function(self, dtime)
        ...
        if self.target_chest then
            ...
            if tname ~= "civi_storage:chest" and tname ~= "civi_storage:chest_locked" and
                ...
                local chest_center = { x = self.target_chest.x + 0.5, y = self.target_chest.y + 0.5, z = self
                .target_chest.z + 0.5 }
                local d2d = vector.distance({ x = pos.x, y = 0, z = pos.z }, { x = chest_center.x, y = 0, z =
                chest_center.z })
                local dy = math.abs(pos.y - self.target_chest.y)

                if npc_debug then minetest.chat_send_all("[NPC-DBG] ChestDist: d2d=" ..
                    string.format("%.2f", d2d) .. " dy=" .. string.format("%.2f", dy)) end
                ...
                if d2d <= 3.5 and dy <= 3.0 then
                    ...
                    if self.deposit_timer < 0.1 then
                        ...
                        if not self.original_chest_name then
                            ...
                            if node.name == "civi_storage:chest" or node.name == "civi_storage:chest_locked" then
                                minetest.swap_node(self.target_chest, { name = node.name .. "_open", param2 = node
                                .param2 })
                                minetest.sound_play("civi_chest_open",
                    ...
                    if self.deposit_timer >= 2.0 then
                        ...
                        if wood_amount > 0 then
                            ...
                            if remaining_wood > 0 then inv:add_item("main",
                                    ItemStack("civi_core:tree " .. remaining_wood)) end
                            self.inv.wood = 0
                        end
                        ...
                        if self.original_chest_name then
                            local node = minetest.get_node(self.target_chest)
                            minetest.swap_node(self.target_chest, { name = self.original_chest_name, param2 = node
                            .param2 })
                            minetest.sound_play("civi_chest_close",
                                { pos = self.target_chest, gain = 0.3, max_hear_distance = 10 })
        ...
        if self.target_tree then
            ...
            if minetest.get_item_group(tnode.name, "tree") == 0 then
                ...
                0.5 }
                local dist_2d = vector.distance({ x = pos.x, y = 0, z = pos.z }, { x = tree_center.x, y = 0, z =
                tree_center.z })
                local dist_y = math.abs(pos.y - self.target_tree.y)

                if npc_debug then minetest.chat_send_all("[NPC-DBG] TreeDist: d2d=" ..
                    string.format("%.2f", dist_2d) .. " dy=" .. string.format("%.2f", dist_y)) end
                ...
                if dist_2d <= 4.0 and dist_y <= 15.0 then
                    ...
                    if self.chopping_timer > 2.0 then
                        ...
                        local nodes = minetest.find_nodes_in_area(p1, p2,
                            { "group:tree", "group:leaves", "group:leafdecay" })
                        ...
                        if npc_debug then
                            ...
                            minetest.chat_send_all("[NPC-DBG] Replant check: " .. sapling_to_plant .. " count=" ..
                            s_count)
                        end
                        ...
                        for _, p_off in ipairs(plant_offsets) do
                            local p_pos = { x = self.target_tree.x + p_off.x, y = self.target_tree.y, z = self
                            .target_tree.z + p_off.z }
                            ...
                            elseif npc_debug then
                                minetest.chat_send_all("[NPC-DBG] Replant FAIL at " ..
                                minetest.pos_to_string(p_pos) ..
                                ": soil=" .. tostring(is_soil) .. " node=" .. minetest.get_node(p_pos).name)
                            end
                        end
            ...
            if (not self.path_way or #self.path_way == 0) and self.path_timer > 3.0 then
                ...
                if pathfinder and pathfinder.find_path then
                    ...
                    minetest.chat_send_all(
                        "[NPC-VITAL] START FINDING PATH from " .. minetest.pos_to_string(rnd_pos) ..
                        " to stand=" .. (self.stand_target and minetest.pos_to_string(self.stand_target) or "nil") ..
                        " to targ=" .. minetest.pos_to_string(target)
                    )

                    self.path_way = pathfinder.find_path(pos, self.stand_target, self)

                    if self.path_way then
                        minetest.chat_send_all("[NPC-VITAL] SUCCESS: Got path of length " .. #self.path_way)
                        self.target_failures = 0
                        self.greedy_timer = 0 -- Reset greedy if path found
                    else
                        minetest.chat_send_all("[NPC-VITAL] FAILED to find path! Failures=" .. (self.target_failures + 1))
        ...
        if target and self.path_way and #self.path_way > 0 then
            ...
            if d2node < threshold then
                ...
                local direction = vector.direction({ x = pos.x, y = 0, z = pos.z }, { x = target_wp.x, y = 0, z =
                target_wp.z })
                        ...
                        for _, chest_pos in ipairs(chests) do
                            ...
                            if self.blacklist[hash] and self.blacklist[hash] >= minetest.get_gametime() then
                                ...
                                if stand_spot then
                                    ...
                                    self.path_timer = 3.1
                                    if not npc_quiet then minetest.chat_send_all("[NPC] Heading to chest at " ..
                                        minetest.pos_to_string(chest_pos)) end
                                    return true
                                else
                    ...
                    if #candidates > 0 then
                        ...
                        for _, found_root in ipairs(candidates) do
                            ...
                            if stand_spot then
                                ...
                                if not npc_quiet then minetest.chat_send_all("[NPC] Nearest unique tree found at " ..
                                    minetest.pos_to_string(found_root)) end
                                return true
                            else
